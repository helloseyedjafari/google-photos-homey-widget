...
# 📸 Google Photos Slideshow for Homey Pro
...
Then on your dashboard: **Edit → add widget → Google Photos Slideshow**, open
...
├── .homeycompose/app.json              # app manifest (CLI compiles app.json)
└── test/googlePhotos.test.js
```

> The Homey CLI generates the compiled `app.json` from `.homeycompose/` and
> `widgets/` during `run`/`install`, so there's intentionally no `app.json`
> committed.

## 📦 Publishing to the App Store (optional)

For personal use, `homey app install` is enough. To publish, add real store
screenshots (`assets/images/*` — placeholders are generated here), widget
